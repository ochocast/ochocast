# Backend Diamond Structure Documentation
...
- `tag.ts`: Defines the business entity for tags.

---

### 2. `infra/`
This folder contains the concrete implementation of interfaces defined in `domain/` as well as the management of external interactions.
...
- **`gateways/`**: Contains implementations of interfaces defined in `domain/gateways/`.
  ...
  - `tag.gateway.ts`: Concrete implementation of the gateway for tags.

---

### 3. `tags.module.ts`
This file groups and exports the different elements related to tags as a module to be used in the application.

![Diamond structure example](/img/Hex-Architecture-Tags.png)

## Example: adding modification
To add a new usecase allowing tag modification, here are the steps to follow:

### 1. Add the usecase in `domain/usecases/`
Create an `updateTag.usecase.ts` file and implement the tag modification business logic.
...
Create an `update-tag.dto.ts` file to define the data format received for modification.

### 5. Modify the controller in `infra/controllers/`
In `tags.controller.ts`, add a new `PUT /tags/:id` endpoint that receives DTO data and calls the update usecase.

### 6. Module update
In `tags.module.ts`, register the new usecase and make it available to the application.

## Conclusion
The diamond architecture offers a strict separation between business logic (`domain`) and infrastructure (`infra`).
- **Use cases** only manipulate entities and abstract gateways.
- **Concrete implementations** are found in `infra`.
