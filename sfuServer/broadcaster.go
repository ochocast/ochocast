...
func NewTrackBroadcaster(track *webrtc.TrackRemote) *TrackBroadcaster {
	tb := &TrackBroadcaster{
		track:    track,
		viewers:  make(map[string]*viewerWriter),
...
func (tb *TrackBroadcaster) broadcastLoop() {
	for {
		...
		case packet, ok := <-tb.packetCh:
			...
			tb.mu.RLock()
			for viewerID, vw := range tb.viewers {
				select {
...
func (tb *TrackBroadcaster) AddViewer(viewerID string, localTrack *webrtc.TrackLocalStaticRTP) {
	...
	// Create a new map with all existing viewers plus the new one
	newViewers := make(map[string]*viewerWriter, len(tb.viewers)+1)
	for k, v := range tb.viewers {
		newViewers[k] = v
	}
	newViewers[viewerID] = vw
	
	// Swap the map atomically
...
func (tb *TrackBroadcaster) RemoveViewer(viewerID string) {
	...
	// Check if viewer exists and get reference before removal
	vw, exists := tb.viewers[viewerID]
	if !exists {
		tb.mu.Unlock()
		return
	}
	
	// Create a new map without the removed viewer
	newViewers := make(map[string]*viewerWriter, len(tb.viewers)-1)
	for k, v := range tb.viewers {
		if k != viewerID {
			newViewers[k] = v
...
func (tb *TrackBroadcaster) Stop() {
	...
	for viewerID, vw := range tb.viewers {
		close(vw.stopCh)
		close(vw.packetCh)
		log.Printf("[BROADCASTER] Stopped writer for viewer %s on track %s", viewerID, tb.track.ID())
	}
	tb.mu.Unlock()
}
