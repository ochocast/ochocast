...
describe('CloseTracktUsecase', () => {
  let closeTrackUseCase: CloseTrackUsecase;
  ...
  it('should throw if trackGateway.updateTrack fails', async () => {
    ...
    await expect(
      closeTrackUseCase.execute(trackId, mockUser.email),
  ...
  it('should throw if the track does not exist', async () => {
    await expect(
      closeTrackUseCase.execute('wrong-id', mockUser.email),
  ...
  it('should throw if the current user does not exist', async () => {
    await expect(
      closeTrackUseCase.execute(trackId, 'wrong@email.com'),
    ).rejects.toThrow(NotFoundException);
  });
  ...
  it('should throw if the current user does not have acces update track', async () => {
    await expect(
      closeTrackUseCase.execute(trackId, mockUser2.email),
