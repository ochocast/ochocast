...
const fetchEventsNotPublished = async () => {
    try {
        let res = await fetch("http://localhost:3001/api/events?published=false")
...
const fetchEventsPublished = async () => {
    try {
        let res = await fetch("http://localhost:3001/api/events?published=true")
        let finalRes = await res.json()
        return finalRes;
    } catch (error) {
        console.error(`Failed to fetch nonpublished events: ${error}`)
    ...
    <div className="events">
        ...
        <div className="content">
            {eventsPublished && (eventsPublished.length >= 1) ?
                <EventsList eventStatus={EventStatus.Published} title="Published Events" events={eventsPublished}/> : null}
            {eventsUnpublished && (eventsUnpublished.length >= 1) ?
                <EventsList eventStatus={EventStatus.NotPublished} title="Not Published Events" events={eventsUnpublished}/> : null}
        </div>
