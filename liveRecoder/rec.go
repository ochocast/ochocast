...
// Parameters:
//   - eventID: unique identifier for the event (e.g. "event-123")
//   - roomID: SFU room ID (e.g. "room-123")
//   - roomKey: SFU room authentication key (can be empty if auth disabled)
//   - sfuBaseURL: SFU base URL (e.g. "http://localhost:8090")
//   - outputDir: directory to save recordings (e.g. "./recordings")
//
// Returns: initialized RecordingSession or error
func NewRecordingSession(eventID, roomID, roomKey, sfuBaseURL, outputDir string) (*RecordingSession, error) {
	...
	mp4FilePath := filepath.Join(outputDir, fmt.Sprintf("%s_%s.mp4", eventID, timestamp))
...
func (rs *RecordingSession) StartRecording() error {
	...
	// Build viewer URL with room_id and optional key
	viewerURL := fmt.Sprintf("%s/viewer?room_id=%s", rs.sfuBaseURL, rs.roomID)
	if rs.roomKey != "" {
		viewerURL = fmt.Sprintf("%s&key=%s", viewerURL, rs.roomKey)
	}
	log.Printf("Viewer URL: %s", viewerURL)

	resp, err := http.Post(viewerURL, "application/sdp",
...
func main() {
	// Command-line flags
	roomID := flag.String("room", "live-event-001", "SFU room ID to record")
	roomKey := flag.String("key", "", "SFU room key (empty if auth disabled)")
	sfuBaseURL := flag.String("sfu", "http://localhost:8090", "SFU base URL")
	outputDir := flag.String("output", "./recordings", "Output directory for recordings")
	flag.Parse()

	log.Println("Live Recorder starting...")
	...
	session, err := NewRecordingSession(*roomID, *roomID, *roomKey, *sfuBaseURL, *outputDir)
	if err != nil {
